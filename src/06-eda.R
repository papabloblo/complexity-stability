...
loglik_stability <- read_csv("paper1/data/results/00-loglik_stability.csv")


files <- list.files("paper1/data/results/complex_measures", full.names = TRUE)
df <- map_df(files, function(x)readRDS(x))

df$dataset <- list.files("paper1/data/results/complex_measures")
...
loglik_stability %>% 
  ggplot(aes(x = overlapping.F1v.mean, y = stability_max_stability)) +
  geom_point()
